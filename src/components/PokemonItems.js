    ...
    render() {
        ...
        return( 
            ...
            <>
                ...
                <Table columns={[
                    {
                        ...
                        "render": (item) => {
                            return(
                                <><Tooltip title={item} mouseLeaveDelay={1}>{item === "Other" || item === "Nothing" ? "" : <Avatar src={"https://img.pokemondb.net/sprites/items/" + item.replace(" ","-").toLowerCase() + ".png"} size={32} />}</Tooltip> {item}</>
                            )
                        }
                    },
                ...
                dataSource={
                    Object.keys(this.state.items || {}).map((v, i) => {
