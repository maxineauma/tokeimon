  ...
  async componentDidMount() {
    this.setState({
      sprite:
        ...
        this.state.style +
        "/normal/" +
        this.spriteNameFormat(this.props.name).toLowerCase() +
        ".png",
      rank: JSON.stringify(pokemonData[this.props.name]["rank"]),
      usage_p: parseFloat(
        JSON.stringify(pokemonData[this.props.name]["usage_percent"])
      ).toFixed(2),
